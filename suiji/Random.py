# -*- coding: utf-8 -*-
"""三维随机凹凸型骨料建模-圆柱体基体-cylinder basic"""
...
AggRatio = 0.01                                      # 骨料体积比
TargetVol = VolumeCy * AggRatio                     # 给定生成的骨料体积，mm³
    ...
    for j in range(Numlayers):
        ...
        if abs(j - 0.0) <= 10e-10:                   # 确定层数，j=0，第一层
            for i in range(NumNodeslayer1st):
                ...
                RamNode[ik][j][2] = theta + setzero * bvar * (-1 + 2 * random.rand())    # (-1 + 2 * random.rand())随机生成(-1,1)的浮点数
            ...
            for i in range(NumNodeslayer5th):  # NumNodeslayer5th = 1，这层的节点数为1个
                ...
                RamNode[ik][(Numlayers - 2) * NumNodeslayer2nd + 1][2] = theta + setzero * bvar * (-1 + 2 * random.rand())
