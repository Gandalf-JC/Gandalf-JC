# -*- coding: utf-8 -*-
"""三维尺度模型---随机凹凸型骨料建模-立方体基体-cube basic"""
...
# rectangular region
ConcLength = 74.0                                 # 长方体混凝土长度，mm
ConcWidth  = 74.0                                 # 长方体混凝土宽度，mm
ConcHeight = 74.0                                 # 长方体混凝土高度，mm
...
for ii in range(MaxIter):
        if abs(len(RamAggDatas)-Numaggs) <= 10e-10:
            break
        x1 = AMaxlen[IndexAR[ik]][0] + random.rand()*(ConcLength- 2*AMaxlen[IndexAR[ik]][0])      # 质心的x坐标，mm
        y1 = AMaxlen[IndexAR[ik]][0] + random.rand()*(ConcWidth - 2*AMaxlen[IndexAR[ik]][0])      # 质心的y坐标，mm
        z1 = AMaxlen[IndexAR[ik]][0] + random.rand()*(ConcHeight- 2*AMaxlen[IndexAR[ik]][0])      # 质心的z坐标，mm
