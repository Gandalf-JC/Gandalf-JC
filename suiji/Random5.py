...
Jobname = 'RamModel-1'
os.chdir(r"E:\Abaqus-test\0923")        # 设置工作目录
...
mdb.models[Modelname].rootAssembly.translate(instanceList=('yt', ), vector=(0, 0, 150.0))
mdb.models[Modelname].rootAssembly.rotate(instanceList=('yt', ), axisPoint=(-50.0, 0.0, 150.0), axisDirection=(
...
mdb.models[Modelname].rootAssembly.rotate(instanceList=('db', ), axisPoint=(-50.0, 0.0, 0.0), axisDirection=(
    100.0, 0.0, 0.0), angle=90.0)
mdb.models[Modelname].rootAssembly.translate(instanceList=('db', ), vector=(0.0, 0.0, -10.0))
...
for ik in range(Numaggs):
    ...
    Volume1 = 0.001                              # 0号粒径体积分数
    Diameter2 = np.random.uniform(9.5, 13.2)  # 1号粒径（9.5-13.2）mm
    Volume2 = 0.001                              # 1号粒径体积分数
    Diameter3 = np.random.uniform(4.75, 9.5)  # 2号粒径粒径（4.75-9.5）mm
    Volume3 = 0.001                              # 2号粒径体积分数
    Diameter4 = np.random.uniform(3.0, 4.75)  # 3号粒径粒径（2.36-4.75）mm
...
mdb.models[Modelname].materials['asphalt-mortar'].Elastic(table=((500.0, 0.30),))
...
for i in range(len(RamAggDatas)):
    ...
    mdb.models[Modelname].rootAssembly.InstanceFromBooleanCut(name=updatepart2,                # 命名
        ...
        cuttingInstances=(mdb.models[Modelname].rootAssembly.instances['Part-' + str(i)], ),   # 切割去掉的实例（抠除的骨料）
        originalInstances=SUPPRESS)                                                            # 原始实例，默认：抑制
    Cumupart.append(mdb.models[Modelname].rootAssembly.instances['Part-' + str(i)])

# # 合并（边）几何实例,为了利于骨料与沥青砂浆的网格划分边界处理。
...
mdb.models[Modelname].rootAssembly.InstanceFromBooleanMerge(name='Part-mortar', instances=Cumupart, keepIntersections=ON,
